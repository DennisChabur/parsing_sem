...
- Используйте проекцию для отображения только полей "properties.lightcond" и "properties.weather"
для документов с "properties.fatalities" равным "Yes".
'''

'''
- Используйте операторы $lt и $gte для подсчета количества документов с "properties.month" меньше 6 
и больше или равно 6, соответственно.
- Используйте оператор $regex для подсчета количества документов, содержащих слово "rain" в поле
 "properties.weather", игнорируя регистр.
- Используйте оператор $in для подсчета количества документов, в которых "properties.rdclass" является 
либо "US ROUTE", либо "STATE SECONDARY ROUTE".
- Используйте оператор $all для подсчета количества документов, в которых "properties.rdconfigur" содержит
 как "TWO-WAY", так и "DIVIDED".
...
print(f'num of docs with fatalities:{collection.count_documents(query)}')

query = {'properties.fatalities': 'Yes'}
proection = {'properties.lightcond': 1, 'properties.weather': 1, '_id': 0}
project_docs = collection.find(query, proection)

for doc in project_docs:
    print(doc)

query = {'properties.month': {'$lt': '6'}}
print(f'num of accidents in less then 6 months: {collection.count_documents(query)}')

query = {'properties.month': {'$gte': '6'}}
print(f'num of accidents in more then 6 months: {collection.count_documents(query)}')

query = {'properties.weather': {'$regex': 'rain', '$options': 'i'}} #- i - отвечает за регистр
#{'$regex': '[Rr]ain'}}
print(f'num of accidents in rain: {collection.count_documents(query)}')
